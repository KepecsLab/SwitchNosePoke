...
if isempty(fieldnames(TaskParameters))
    %general
    TaskParameters.GUI.Ports_LMR = '128';
    ...
    TaskParameters.GUIPanels.General = {'Ports_LMR','Ports_Switch', 'FI','VI','ChoiceDeadline','LightGuided'};
    ...
    TaskParameters.GUI.ForceSwitch =0;
    TaskParameters.GUIMeta.ForceSwitch.Style = 'checkbox';
    
    TaskParameters.GUI.AlwaysSwitch =0;
    TaskParameters.GUIMeta.AlwaysSwitch.Style = 'checkbox';
    
    TaskParameters.GUI.MVT =0;
    TaskParameters.GUIMeta.MVT.Style = 'checkbox';
    ...
    TaskParameters.GUIPanels.Reward = {'rewardAmount','CenterPortRewAmount','SwitchPortRewAmount','ErrorSwitch','ForceSwitch','AlwaysSwitch', 'MVT','CenterPortProb','RewardProb','Deplete','DepleteRate','Jackpot','JackpotMin','JackpotTime'};
...
BpodSystem.Data.Custom.ChoiceSwitch = 0;
BpodSystem.Data.Custom.BaitedSwitch = 0;
...
BpodSystem.Data.Custom.SwitchPortRewAmount =TaskParameters.GUI.SwitchPortRewAmount;
...
%Defines switch wait action 
if BpodSystem.Data.Custom.BaitedSwitch(iTrial) 
    SwitchWaitAction='water_Sw';
else
    if TaskParameters.GUI.ErrorSwitch
        SwitchWaitAction='ErrorSwitch';
    else
         SwitchWaitAction='ITI';
    end
end
...
elseif TaskParameters.GUI.Jackpot ==4 % 
 ...
 %center poke out transitions to wait_Sin and has 4 exit conditions - left,
 %right, switch, or time up
 ...
 if TaskParameters.GUI.ForceSwitch && BpodSystem.BaitedSwitch(iTrial)
    ...
    'Timer',0,...
    'StateChangeConditions', {SwitchPortIn,'wait_Sw_start'},...
...
sma = AddState(sma, 'Name', 'water_Sw',...
    'Timer', SwitchValveTime,...
    'StateChangeConditions', {'Tup','ITI'},...
    'OutputActions', {'ValveState', SwitchValve});
...
%ErrorSwitch is a GUI TaskParameter that determineentry into the
%ErrorSwitch state wherein the rat is punished for a "lean" or
%disadvantageous switch. If ErrorSwitch is off then a "lean" switch
%transitions to ITI. LeanSwitch is a housekeep parameter for entry into
%ErrorSwitch state. BpodSystem.Data.Custom.SwitchAvailable and 
%BpodSystem.Data.Custom.BaitedSwitch determine lean/rich switches
...
if any(strncmp('wait_L',statesThisTrial,6))
    ...
    BpodSystem.Data.Custom.ChoiceSwitch(iTrial)=0;
...
elseif any(strncmp('wait_Sw',statesThisTrial,7))
...
elseif any(strcmp('ErrorSwitch',statesThisTrial))
    BpodSystem.Data.Custom.LeanSwitch(iTrial) = true;
end
...
elseif any(strncmp('water_R',statesThisTrial,7)) 
    BpodSystem.Data.Custom.Rewarded(iTrial) = true;
elseif any(strncmp('water_Sw',statesThisTrial,7)) 
    BpodSystem.Data.Custom.Rewarded(iTrial) = true;
end
...
%reward depletion - change to monotonic depletion
if BpodSystem.Data.Custom.ChoiceLeft(iTrial) == 1 && TaskParameters.GUI.Deplete
    ...
    BpodSystem.Data.Custom.RewardMagnitude(iTrial+1,2) = BpodSystem.Data.Custom.RewardMagnitude(iTrial,2);
...
%Always switch condition makes BaitedSwitch always true(i.e., switching
%will always be rewarded and always reset L and R. ForceSwitch forces the
%swithc s.t. going tothe switch port is the only way to continue the trial.
%InMVT switch port isalways available, but baited is only true when the
